#!/usr/bin/env python3
"""
TFLite Exporter using ai-edge-torch for XNNPACK-compatible models.
This script exports student policy models without requiring Flex delegate.
"""
...
def export_to_tflite(model, output_path, num_observations):
    ...
    print(f"Exporting model to TFLite using ai-edge-torch...")
...
def main():
    parser = argparse.ArgumentParser(description='Export student policy to TFLite format using ai-edge-torch')
    parser.add_argument(
        '--input', '-i',
        type=str,
        default='model/student_future_keypoints.pt',
